...
class FormGestionFormateur:
    def __init__(self):
          ...
          self.btnShow=tk.Button(self.frameBUtten,text="show",command=self.Show)
    ...
    def Show(self):
           fonetre=tix.Tk()
           fonetre.title("List Des Stagire")
           fonetre.geometry("300x300")
           sw=tix.ScrolledWindow(fonetre)
           sw.pack(expand=1,fill=tix.BOTH)
           i=0
           for item in self.etab1.getUsers():
                 if isinstance(item ,Formateur):
                       entyLogin=tix.Entry(sw.window)
                       entyLogin.grid(row=i,column=0)
                       entyLogin.insert(0,item.getLogin())
                       entyPwd=tix.Entry(sw.window)
                       entyPwd.grid(row=i,column=1)
                       entyPwd.insert(0,item.getPwd())
                       entyMatricule=tix.Entry(sw.window)
                       entyMatricule.grid(row=i,column=2)
                       entyMatricule.insert(0,item.getMatricule())
                       i+=1
           fonetre.mainloop()
